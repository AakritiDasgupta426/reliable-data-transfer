...
if __name__ == '__main__':
	...
	bdp_window = max(8, int(bdp/chunk_size))
	config_window = int(cfg.get('sender', 'window_size'))
	window_size = max(1, min(bdp_window, config_window))
	#print(f"Sender : bdp={bdp:.0f} bdp_window: {bdp_window} config_window {config_window} using window = {window_size}")
	#send_monitor.socketfd.settimeout(timeout)
